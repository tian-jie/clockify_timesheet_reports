...
namespace Innocellence.Web.Controllers
{
    ...
    public class AccountController : ParentController<SysUser, SysUserView>
    {
        ...
        public ILogsService LogService { get; private set; }
  ...
  (System.Web.Configuration.AuthenticationSection)System.Web.Configuration.WebConfigurationManager.GetSection("system.web/authentication");
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            {
                if (CanLogin(model.UserName))
                {
                    var user = UserManager.UserLoginAsync(model.UserName, model.Password);
                    if (user != null)
                    {
                        await authService.SignInAsync(user, model.RememberMe);

                        //登录日志
                        LogService.Repository.Insert(new LogsModel() { LogCate = "AdminLogin", LogContent = "登录成功", CreatedUserName = model.UserName, LogSource = Request.UserHostAddress });
                        return Json(doJson(null, returnUrl), JsonRequestBehavior.AllowGet);
                    }
                    else
                    {
                        LogService.Repository.Insert(new LogsModel() { LogCate = "AdminLogin", LogContent = "登录失败", CreatedUserName = model.UserName, LogSource = Request.UserHostAddress });
                ...
                {
                    ModelState.AddModelError("", "失败次数过多，请稍后重试。");
                    return Json(GetErrorJson(), JsonRequestBehavior.AllowGet);
                }
            }
        ...
        private bool CanLogin(string userName)
        {
            if (!string.IsNullOrWhiteSpace(userName))
            {
                ...
                DateTime edge = now.AddMinutes(-15);
                int failedCount = LogService.Repository.Entities.Count(q => userName.Equals(q.CreatedUserName, StringComparison.OrdinalIgnoreCase) && "登录失败".Equals(q.LogContent) && q.CreatedDate.HasValue && q.CreatedDate.Value >= edge && q.CreatedDate.Value < now);
                if (failedCount < 5)
                {
                    return true;
                }
