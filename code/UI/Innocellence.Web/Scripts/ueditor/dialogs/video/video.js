...
(function(){
    ...
    function insertUpload() {
        ...
        for(var key in uploadVideoList) {
            ...
            var theVideo = $('#'+ file.fileId).find("video");
        ...
        $.ajax({
            type: 'post',
            async: false,
            url: '/WeChatMain/ArticleInfo/GetPosterUrlList',
            data: { "posterUrlList": posterlist },
            success: function (result) {

                for (var i = 0 ; i < result.length; i++) {
                    for (var j = 0; j < videoObjs.length; j++) {
                        if (result[i].fileId == videoObjs[j].fileId) {
                            videoObjs[j].poterUrl = "/" + result[i].filePath
                        }
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                ...
                try {
                    ...
                    if (json.state == 'SUCCESS') {
                        ...
                        $file.find(".imgWrap").html('<video class="previewVideo"' +
                                                    ' src="' + json.url + '"' +
