...
Innocellence.menu = {
    ...
    init: function () {
        ...
        Innocellence.menu.token = $('#tokenStr').val();

        $("#buttonDetails_type").change(Innocellence.menu.typeChanged);
            ...
            $.getJSON('/Menu/GetToken?t=' + Math.random(), { appId: $('#menuLogin_AppId').val(), appSecret: $('#menuLogin_AppSecret').val() },
                function (json) {
                    if (json.access_token) {
                        Innocellence.menu.setToken(json.access_token);
                    } else {
        ...
        $('#menuLogin_SubmitOldToken').click(function () {
            Innocellence.menu.setToken($('#menuLogin_OldToken').val());
        });

        $('#btnGetMenu').click(function () {
            menuState.html('获取菜单中...');
            $.getJSON('./GetMenu?t=' + Math.random(), { token: Innocellence.menu.token }, function (json) {
                if (json.menu) {
        ...
        $('#btnDeleteMenu').click(function () {
            ...
            $.getJSON('./DeleteMenu?t=' + Math.random(), { token: Innocellence.menu.token }, function (json) {
                if (json.Success) {
        ...
        $('#submitMenu').click(function () {
            ...
            var postData = $("#form_Menu").serializeArray();
            var strUrl = $('#form_Menu')[0].action;
