jQuery(function() {
	...
	uploader = WebUploader.create({
		...
		accept: {
			title: 'Files',
			extensions: 'gif,jpg,jpeg,bmp,png,mp4,avi,mov',
		...
		fileNumLimit: 5,
		fileSizeLimit: 200 * 1024 * 1024 * 1024,
	...
	function addFile(file) {
		var $li = $('<li id="' + file.id + '">' + '<input type="hidden" name="images[]" value="' + file.name + '">' + '<p class="imgWrap"></p>' + '<p class="progress"><span></span></p>' + '</li>'),
	...
	function updateStatus() {
		...
		if (state === 'ready') {
			...
			if (stats.uploadFailNum) {
				text = '已成功上传' + stats.successNum + '个文件至后台，' + stats.uploadFailNum + '个文件上传失败，<a class="retry" href="#">重新上传</a>失败文件或<a class="ignore" href="#">忽略</a>'
			}
