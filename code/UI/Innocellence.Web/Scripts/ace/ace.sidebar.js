...
(function ($, undefined) {
    ...
    function Sidebar(sidebar, options) {
        ...
        this.is_collapsible = function () {
			...
			&& toggle.scrollHeight > 0; //sidebar is collapsible and collapse button is visible?
        };
        ...
        this.$sidebar.on(ace.click_event + '.ace.submenu', '.nav-list', function (ev) {
            ...
            var sub_hidden = (sub.scrollHeight == 0); //if not open and visible, let's open it and make it visible
        ...
        this.show = function (sub, $duration, wait) {
            ...
            var complete = function (ev, trigger) {
                ...
                $sub
				.css({ 'transition-property': '', 'transition-duration': '', overflow: '', height: '' }); //if(ace.vars['webkit']) ace.helper.redraw(sub);//little Chrome issue, force redraw ;)
    ...
    //menu-toggler
    //TODO:remove click event namespace of sildebar for fixing  issue no #14531  by john
    $(document)
	...
	.on(ace.click_event , '.sidebar-collapse', function (e) {
