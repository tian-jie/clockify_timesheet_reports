...
(function($ , undefined) {
	...
	var Widget_Box = function(box, options) {
		...
		this.update = function () {
		    ...
		    var controls = $body.find('.edit-control, .edit-label');
		    controls.each(function (i) {
		        $(this).toggleClass('hidden');
		    });

		    $body.find('.select2').width('100%');

		}

		this.addOrUpdate=function(strModalID,strID){
		    var modal = $('#' + strModalID).data('modal');
		   // $('#' + strModalID).data('modal', modal);
		   modal.ShowUpdateInfo(strID);
		    // $('#' + strModalID).modal();
		}
		...
		this.save = function () {
		    ...
		    var postData = $form.serializeArray();
             this.reload();
		    $.post($form[0].action, postData, function (data) {
		        if (ReturnValueFilter(data)) {
		            var controls = $body.find('.edit-control, .edit-label');
		            controls.each(function (i) {
		                $(this).toggleClass('hidden');
		               var $group= $(this).closest('.form-group');
		               var eles = $(this).find('input,select,textarea');
		               $.each(eles, function (i, item) {
		                   var c = $group.find('#label-' + item.name);
		                   var html = '';
		                   if (item.tagName == 'INPUT') {
		                       html = $(item).val();
		                   }
		                   else if (item.tagName == 'SELECT') { html = $(item).find("option:selected").text(); }
		                   else if (item.tagName == 'TEXTAREA') { html = $(item).val().trim().replace(/\n/g,'<br/>'); }
		                   $(c).html(html);

		                   var dis=$(item).data('display');
		                   if (dis) { window[dis](item, c); }
		            ...
		            $box.trigger('saved.ace.widget');

		            $box.trigger('reloaded.ace.widget');

		            $box.find('.widget-header a[data-action=update]').removeClass('hidden');
		            $box.find('.widget-header a[data-action=save],.widget-header a[data-action=cancel]').addClass('hidden');

		        } else { $box.trigger('reloaded.ace.widget'); return false; }
		    });
	...
	$(document).on('click', '.modal-footer button[data-action]', function (ev) {
	    ev.preventDefault();
	    var $this = $(this);
	    ...
	    if ($action == 'save') {
	        ...
	        var model = $modal.data('modal');
	        model.AddOrUpdateInfo($form[0].action);
	...
	$(document).on('click.ace.widget', '.widget-header a[data-action]', function (ev) {
		...
		if($action == 'collapse') {
		    ...
		    $widget_box.update();
		    $(this).closest('.widget-toolbar').find('a[data-action=save],a[data-action=cancel]').removeClass('hidden');
		    ...
		    // $widget_box.save();
		    $toolbar=  $(this).closest('.widget-toolbar');
		    $toolbar.find('a[data-action=save],a[data-action=cancel]').addClass('hidden');
		    $toolbar.find('a[data-action=update]').removeClass('hidden');
		    ...
		    $widget_box.addOrUpdate($this.data('target'),0);
		}
